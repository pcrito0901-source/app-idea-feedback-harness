...
- `docs/glossary.md`（用語集。ユーザー向けの説明はここと同じ言葉を使う）
- `docs/rounds/round-N/` の7ファイル（今回のラウンドの分析全文）
  **プロンプトへの貼り付けが途中で欠けている場合は、必ずここから読んで補完する。**
  7ファイルすべてに目を通す前に判断を始めてはならない
...
**記入例・プレースホルダの行は数えない**（日付が `YYYY-MM-DD` のまま、合計点が空欄、`（記入例）` と書かれている行）。
`docs/rounds/` に `round-N` ディレクトリが何個あるかでも確認できる。
...
Cap を無視した高得点は禁止。初回ラウンドの **最終点** が40点前後になるのは正常。

**2つの合計を必ず両方出す。片方だけを出してはならない。**

- **案の強さ（素点合計）** — Cap をかけない。**筋の通った案なら初回でも80点を超えてよい。低く見せない**
- **確かめた度合い（最終点合計）** — Cap 適用後。**判定に使うのはこちら**

素点を不当に低く付けて「Cap があるからどうせ同じ」と済ませてはならない。
**素点は案の形そのものへの評価であり、ここを手抜きすると、
「筋は通っているが未確認」と「案の形自体が弱い」の区別がつかなくなる。**

配点の 40% 未満の項目は **Critical Risk** としてマークする（判定は **最終点** で行う）。
...
## この案の強いところ
（**必ず書く。** 平易な言葉で1〜3個。お世辞ではなく、**根拠のある事実だけ**を書く）

**情報源は6つある。すべて確認してから書くこと。**

1. Critic の「崩せなかった点」
2. customer-researcher の「最も強い点（顧客・問題の観点）」
3. product-strategist の「最も強い点（プロダクトの観点）」
4. growth-strategist の「最も強い獲得経路」
5. business-strategist の「最も強い収益の芽」
6. technical-analyst の「技術的に有利な点」

**「今回は1つも無かった」と書けるのは、6つすべてが空だったときに限る。**
1つでも埋まっているのにこの節を空にしたら、それは分析の欠落である。

**弱点と同じ厳密さで書く。** 強みにも根拠があり、証拠が弱いなら弱いと分かる書き方にする
（第1部では `L0` という記号は使わず、「まだ確かめていませんが」のような平易な言葉に直す）。

## 今の評価

| | 点数 | 意味 |
|---|---|---|
| **案そのものの筋** | N / 100 | （1行。**まだ誰にも確かめていない、AIの見立てであることを明記する**） |
| **確かめた度合い** | N / 100 | （1行。上限が今いくつで、何をすれば上がるか） |

（**2つ並べる。片方だけは禁止。** `素点` `最終点` `Evidence Cap` という用語は使わず、
 上の日本語の見出しで書く。**低いほうだけを強調しない**）
...
- **スコアの内訳（項目別の点数・Cap・素点の計算過程）を書かない。** 内訳は第2部に置く
- ただし **2つの合計だけは第1部に書く**（下の「今の評価」）。
  片方だけを書いてはならない。**「確かめた度合い」だけを見せると、
  筋の通った案が低く評価されたように誤読される**
- `L0` `L2` `CRITICAL` `Evidence Cap` などのハーネス用語を使わない
- **`OPP-1` `EXP-001` `A1` `v002` のような ID を書かない。** 内容を日本語で書く（例:「手入力をやめて写真から読み取る案」）
...
## Strongest Point
（現在のアイデアで最も強い部分。**6つの情報源すべてから拾う** —
 Critic の「崩せなかった点」と、5体それぞれの「最も強い点 / 獲得経路 / 収益の芽 / 有利な点」）

| 強い点 | 出どころ | Evidence Level |
|---|---|---|
| ... | critic / customer / product / growth / business / technical | L0 |

**5体が挙げた強みを、Critic が反証していない場合は、そのまま採用してよい。**
Critic が反証した強みは、その反証と併記する。
...
- **確かめた度合い（最終点合計）**: N / 100 — 判定に使うのはこちら
- **読み方**: （高×低 =「筋は通っている。足りないのは確認だけ」/ 低×低 =「案の形に問題がある」/
  高×高 =「BUILD の候補」/ 低×高 =「確かめた結果、否定された」のどれか）

**前回比**: 案の強さ +N / 確かめた度合い +N（初回の場合は「初回」）
...
## このラウンドで使った用語
（第2部で実際に使った専門用語・略語だけを一覧にする。**使っていない用語は載せない。**
 説明は `docs/glossary.md` と同じ言葉を使う。`docs/glossary.md` に無い用語を使った場合は、
 ここに書いたうえで `docs/glossary.md` にも追記する）

| 用語 | 意味（1行・普通の日本語で） |
|---|---|
| Retention | 2回目以降も使い続けてもらえるか |
| Distribution | 作った後に、どうやって知ってもらうか |
| L0 | まだ誰にも確かめていない、AIの推測だけの状態 |
...
第2部は記録用なので専門用語を使ってよい。ただし `CLAUDE.md` の「用語と ID の書き方」に従う。

- **ID を裸で書かない。** 表でも `OPP-1「手入力をやめて写真から読み取る」` のように内容を併記する
- **そのラウンドで初めて使う用語には、括弧書きで1行の言い換えを添える。** 2回目以降は省いてよい
- 末尾の「このラウンドで使った用語」を省略しない
...
5. **第1部に、説明のない専門用語や ID（`OPP-1` `L0` `CRITICAL` など）が1つでも出ていないか。** 出ていたら書き直す
6. **「で、結局何が言いたいのか」が1文で答えられるか。** 答えられないなら、結論が定まっていない。書き直す
...
### 9. `docs/glossary.md`（毎ラウンド）

このラウンドで **新しく使った専門用語** を追記する。既にある用語は触らない。

- 説明は「普通の日本語で1〜3行」。用語で用語を説明しない（「Retention とは継続率のことです」は説明になっていない）
- 該当する章（判定 / 証拠 / 案の中身 / ID / 改善の方向 / 検証 / 仕組み）に入れる
- 新しい用語が無かったラウンドは、何もしなくてよい

**この更新を省略しない。** 説明されない用語が増えていくと、ユーザーは自分の案の判定結果を読めなくなる。
