...
13. **根拠のある強みを、書かずに済ませない。** 弱点だけを並べた分析は、肯定だけの分析と同じくらい役に立たない（→ 下記）

### 「厳しさ」は目的ではない

上のルール1は **無条件の肯定** を禁じているのであって、**肯定そのもの** を禁じてはいない。
ここを取り違えると、ハーネスは「何を言っても否定が返ってくる装置」になり、判断材料として使えなくなる。

- 根拠のある強みは、**弱点と同じ厳密さで、同じ必須枠で報告する**
- 強みにも Evidence Level を付ける。**L0 の強みは L0 と書く**（お世辞にしない。だが黙殺もしない）
- **弱点だけを並べた分析は、肯定だけの分析と同じくらい役に立たない。** どちらも意思決定に使えない
- **作り手が既に持っている資産**（接触経路、加入済みのサービス、使える開発手段）は、
  ゼロから用意する場合より明確に有利である。**それを強みとして正しく数える**
- 案が本当に強いなら、そう書く。それは甘さではなく **精度** である

構造上の担保:

| 誰が | 何を必ず書くか |
|---|---|
| 5つの専門 Agent | それぞれの領域の **「最も強い点」**（リスクの節と対になる必須枠） |
| `@critic` | **「崩せなかった点」** と **悲観バイアスの自己検査** |
| `@idea-chair` | 第1部の **「この案の強いところ」**（上記6つの情報源すべてを確認してから書く） |

**強みの節を空にできるのは、6つの情報源すべてが空だったときだけである。**
...
- 使える時間・予算・期限
- 収益化の意図（趣味 / 副収入 / 事業）
- **実装をどう行うか**（自分で書く / Claude Code などの AI にほぼ任せる / 併用）。
  `@technical-analyst` の見積もりの前提が変わるため、必ず `docs/idea.md` の「作り手の制約」に記録する
...
**Cap がかかるのは最終点だけである。素点には上限をかけない。**
そして毎ラウンド、**2つの合計を並べて出す。片方だけを出してはならない。**

| 名前 | 計算 | 何を測っているか |
|---|---|---|
| **案の強さ（素点合計）** | 素点の合計 | 案そのものの筋。**良い案なら初回から80点を超えてよい** |
| **確かめた度合い（最終点合計）** | 最終点の合計 | 検証できている量。**`BUILD` の判断に使うのはこちら** |

1つの数字に両方を混ぜると、Round 1 では優れた案も凡庸な案も同じ点数になり、
**数字が何も判別しなくなる。** 分けることで、「筋は通っているが未確認」と
「案の形自体が弱い」を区別できるようになる。

ただし **「案の強さ」は AI の評価であり、それ自体が L0 である。**
高くても「AIが見て筋が通っている」以上の意味は無く、**これを根拠に開発を始めてはならない。**
`BUILD` / `KILL` / Critical Risk の判定に使うのは、常に **最終点** の側である。

到達できる最終点の上限（素点が満点だった場合）:
**L0 = 44点 / L1 = 60点 / L2 = 80点 / L3 = 92点 / L4 = 100点。**
`BUILD` の条件である70点は、**L2（実際のユーザーに話を聞く）で届く。**
...
4. **検証コストが開発コストを上回る場合**、検証せずに小さく作ることを提案してよい（実験は開発より安いときにだけ意味がある）。
   **開発手段が AI 支援の場合、この条件は成立しやすくなる。** ただし成立するのは
   「作れるか」「使いやすいか」を確かめたいときだけである。
   **「欲しがる人がいるか」「お金を払うか」「どうやって知ってもらうか」は、安く作れても答えが出ない。**
   需要・課金・流通の検証を、開発が安いことを理由に飛ばしてはならない
